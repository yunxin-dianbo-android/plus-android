...
public abstract class RichEditor extends WebView {
    ...
    public void setOnDraftLoadFinishListener(OnDraftLoadFinishListener onDraftLoadFinishListener) {
        mOnDraftLoadFinishListener = onDraftLoadFinishListener;
    }
    ...
    private class EditorWebViewClient extends WebViewClient {
        ...
        public void onPageFinished(WebView view, String url) {
            ...
            if (mOnDraftLoadFinishListener != null && SETUP_BASEURL.equalsIgnoreCase(url)) {
                mOnDraftLoadFinishListener.onDraftLoadFinish();
            }
        }
    ...
    private class Android4JsInterface {
        ...
        @JavascriptInterface
        public void onClick() {
            Observable.just(mOnClickListener)
                    .filter(listener -> listener != null)
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribe(OnClickListener::onClick);
        }
        ...
        public void resultWords(String title, String markdown, String noMarkdownWords, String allHtml, boolean isPublish) {
            Observable.just(mOnMarkdownWordResultListener)
                    ...
                    .subscribe(listener -> {
                        ...
                        // 后台只能接受 191 个字
                        // fixed 2018-7-2 11:33:30
                        // mysql 5.7 只接受 5.7 ， remove fixed 2018-7-9 16:49:41，by tym
                        if (result.length() >= 191) {
                            result = result.substring(0, 191);
                        }
