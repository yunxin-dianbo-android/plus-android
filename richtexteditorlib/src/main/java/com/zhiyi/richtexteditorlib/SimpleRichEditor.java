...
public class SimpleRichEditor extends RichEditor {
    ...
    public interface OnEditorClickListener {
        ...
        void onEmojiImageButtionClick(boolean isSelected, boolean showKeyboard);

        void onArrowImageButtionClick();
    }
...
// .link, false));
//        mBottomMenu.setOnItemClickListener(this);
        ...
        if (item.getId() != ItemIndex.EMOJI) {
            if (mBottomMenu.getBottomMenuItems().get(ItemIndex.EMOJI) != null) {
                ...
                if (menuEmoji.getSelected()) {
                    ...
                    if (item.getId() == ItemIndex.INSERT_IMAGE||item.getId() == ItemIndex.HALVING_LINE) {
                        showKeyboard = false;
                    }
                    mOnEditorClickListener.onEmojiImageButtionClick(false, showKeyboard);
                }
            }
        }
    }
    ...
    public SimpleRichEditor addTypefaceBranch(boolean needBold, boolean needItalic, boolean
        ...
        mBottomMenu.addRootItem(getBaseItemFactory().generateItem(getContext(), ItemIndex.A,
                (item, isSelected) -> {
                    ...
                    if (mBottomMenu.getBottomMenuItems().get(ItemIndex.EMOJI) != null) {
                        MenuItem menuEmoji = mBottomMenu.getBottomMenuItems().get(ItemIndex.EMOJI).getMenuItem();
                        if (!isSelected && menuEmoji.getSelected()) {
                            mBottomMenu.getInnerListener().onItemClick(menuEmoji);
                            mOnEditorClickListener.onEmojiImageButtionClick(false,true);
                        }
                        mBottomMenu.getBottomMenuItem(item).setSelected(isSelected);
                    }
                    return false;
    ...
    public SimpleRichEditor addArrow() {
        addRootCustomItem(ItemIndex.ARROW, getBaseItemFactory().generateItem(
                ...
                (item, isSelected) -> {
                    ...
                    mOnEditorClickListener.onArrowImageButtionClick();
                    onItemClick(item);
                    return true;
                }));
        return this;
    ...
    public SimpleRichEditor addEmoji() {
        addRootCustomItem(ItemIndex.EMOJI, getBaseItemFactory().generateItem(
                getContext(),
                ItemIndex.EMOJI,
                (item, isSelected) -> {
                    ...
                    mBottomMenu.getBottomMenuItem(item).setSelected(isSelected);
                    mOnEditorClickListener.onEmojiImageButtionClick(!isSelected,true);
                    return false;
                }));
        return this;
    }
