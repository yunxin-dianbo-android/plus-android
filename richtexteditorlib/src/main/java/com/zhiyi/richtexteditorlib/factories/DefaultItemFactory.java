...
public class DefaultItemFactory extends BaseItemFactory<ImageViewButtonItem>{
    ...
    protected  ImageViewButtonItem generateEmojiImageItem(Context context, IBottomMenuItem.OnBottomItemClickListener listener) {
        ImageViewButtonItem item = generateAutoSetItem(context, ItemIndex.EMOJI, R.drawable.emoji);
        item.setOnItemClickListener(listener);
        return item;
    }
    ...
    public  ImageViewButtonItem generateItem(Context context,Long id,IBottomMenuItem.OnBottomItemClickListener listener) {
        switch (id.intValue()){
            ...
            case (int) ItemIndex.EMOJI:
                return generateEmojiImageItem(context,listener);
