...
public class EmojiAdapter extends PagerAdapter {
    ...
    /**
     * 每个页面最多展示的emoji数量 此处不包括最后一个预留的删除
     */
...
//        this.pageMaxCount = showMaxLines * showMaxColumns - 1;
        this.pageMaxCount = showMaxLines * showMaxColumns;
        initList();
    }

    public EmojiAdapter(Context context, List<List<String>> listSource, List<List<String>> nameSource, int maxViewWidth, int showMaxLines, int showMaxColumns, int emojiSize) {
        this.context = context;
        this.listSource = listSource;
            ...
            for (int i = 0; i < itemMaxIndex; i++) {
                ...
                for (int j = 0; j < pageMaxCount; j++) {
                    ...
                    if (index < listSize) {
                        ...
                        if (nameSource != null) {
                            nameList.add(nameSource.get(listSource.indexOf(list)).get(index));
                        }
                    } else {
                        j = pageMaxCount;
                    }
    ...
    class Holder extends RecyclerView.ViewHolder{
        ...
        public Holder(CustomTextView itemView) {
            super(itemView);
            mTextView= itemView;
        }
    }
    ...
    public Object instantiateItem(ViewGroup container, final int position) {
        final List<String> list = lists.get(position);
        ...
        GridLayoutManager gridLayoutManager = new GridLayoutManager(context, showMaxColumns);
        ...
        recyclerView.setAdapter(new RecyclerView.Adapter<Holder>() {
            ...
            public void onBindViewHolder(Holder holder, int position) {
                ...
                holder.mTextView.setBackground(context.getDrawable(R.drawable.user_select_bg));
                holder.mTextView.setTextSize(emojiSize);
//                holder.mTextView.getLayoutParams().width = maxViewWidth / showMaxColumns;
//                holder.mTextView.getLayoutParams().height = ViewGroup.LayoutParams.WRAP_CONTENT;
                holder.mTextView.setGravity(Gravity.CENTER);
...
//                textView.setBackground(context.getDrawable(R.drawable.user_select_bg));
                ...
                if (i < list.size()) {
                    ...
                    if (name != null) {
                        nametext[0] = name.get(i);
                    }

                    textView.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            emojiOnClick.onClick(finaltext, nametext[0]);
                        }
                    });
                } else {
                    if (i == pageMaxCount) {
    ...
    interface EmojiTextOnClick {
        void onClick(String text, String name);
    }
}