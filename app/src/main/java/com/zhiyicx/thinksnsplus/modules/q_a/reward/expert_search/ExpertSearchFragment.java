...
public class ExpertSearchFragment extends TSListFragment<ExpertSearchContract.Presenter, ExpertBean>
        ...
        RxTextView.editorActionEvents(mFragmentInfoSearchEdittext).subscribe(textViewEditorActionEvent -> {
            if (textViewEditorActionEvent.actionId() == EditorInfo.IME_ACTION_SEARCH) {
                if(TextUtils.isEmpty(mFragmentInfoSearchEdittext.getText())){
                    ...
                    mTvRecommendHint.setVisibility(View.GONE);
                    // 这里如果是需要话题下专家的话，就把第二个参数改为 topic_ids；
    ...
    private void requestNetData(Long maxId, int topicId, boolean isLoadMore) {
        mPresenter.requestNetData(maxId, topicId, isLoadMore);
    }

    private void requestNetData(int size, String topicIds, boolean isLoadMore) {
        mPresenter.requestNetData(size, topicIds, TextUtils.isEmpty(mFragmentInfoSearchEdittext.getText().toString())
