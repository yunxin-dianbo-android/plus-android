 ...
 * @Description
 */
public class CirclePostDetailPresenterNew extends AppBasePresenter<CirclePostDetailContract.View>
    ...
    @Inject
    public CirclePostDetailPresenterNew(CirclePostDetailContract.View rootView) {
        super(rootView);
    }
     ...
     */
    @Override
    public void sharePost(Bitmap shareBitMap, List<UmengSharePolicyImpl.ShareBean> data) {
        CirclePostListBean circlePostListBean = mRootView.getCurrentePost();
    ...
    public void onStart(Share share) {
        ...
        switch (share) {
            case FORWARD:
                letter = new Letter(TSEMConstants.BUNDLE_CHAT_MESSAGE_FORWARD_TYPE_POST);
                if (mRootView.getCurrentePost() == null) {
                    return;
                }
                // 帖子
                StringBuilder content = new StringBuilder(mRootView.getCurrentePost().getFriendlyContent() + "");
                if (mRootView.getCurrentePost().getImages() != null && !mRootView.getCurrentePost().getImages().isEmpty()) {
                    int id = mRootView.getCurrentePost().getImages().get(0).getFile_id();
                    ...
                    for (CirclePostListBean.ImagesBean imagesBean : mRootView.getCurrentePost().getImages()) {
                        content.append(MarkdownConfig.IMAGE_REPLACE);
                    }
                }
                letter.setCircle_id(mRootView.getCurrentePost().getGroup_id() + "");
                letter.setName(mRootView.getCurrentePost().getTitle());
                letter.setContent(content.toString());
                letter.setId(mRootView.getCurrentePost().getId() + "");
                ...
                break;
            case LETTER:
                letter = new Letter(TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_POST);
                if (mRootView.getCurrentePost().getImages() != null && !mRootView.getCurrentePost().getImages().isEmpty()) {
                    int id = mRootView.getCurrentePost().getImages().get(0).getFile_id();
                    String image = ImageUtils.imagePathConvertV2(id, mContext.getResources()
                                    .getDimensionPixelOffset(R.dimen.chat_post_image_widht), mContext.getResources()
                ...
                letter.setCircle_id(mRootView.getCurrentePost().getGroup_id() + "");
                letter.setName(mRootView.getCurrentePost().getTitle());
                letter.setContent(mRootView.getCurrentePost().getFriendlyContent());
                letter.setId(mRootView.getCurrentePost().getId() + "");
