...
public class TopPostItem extends BaseTopItem implements BaseTopItem.TopReviewEvetnInterface {
    ...
    public void convert(ViewHolder holder, BaseListBean baseListBean, BaseListBean lastT, int position, int itemCounts) {
        ...
        String content = "";
        if (!postIsDeleted) {
            content = postListBean.getPost().getSummary();
            imageId = RegexUtils.getImageIdFromMarkDown(MarkdownConfig.IMAGE_FORMAT, content);
            if (hasImage) {
                imageId = postListBean.getPost().getImages().get(0).getFile_id();
            }
        }
        holder.setVisible(R.id.fl_image_container, (hasImage || imageId > 0) ? View.VISIBLE : View.GONE);

        if (hasImage || imageId > 0) {
            holder.setVisible(R.id.fl_image_container, View.VISIBLE);
            String url;
            holder.setVisible(R.id.iv_video_icon, View.GONE);
            url = ImageUtils.imagePathConvertV2(imageId
        ...
        RxView.clicks(reviewFlag)
                .throttleFirst(JITTER_SPACING_TIME, TimeUnit.SECONDS)
                .subscribe(aVoid -> handleReview(position, postListBean));
    ...
    protected void toDetail(CirclePostListBean postListBean, boolean isLookMoreComment) {
        CirclePostDetailActivity.startCirclePostDetailActivity(mContext, postListBean, isLookMoreComment, true);
    }
}
