...
public class DraftBoxPresenter extends AppBasePresenter<DraftBoxContract.View> implements DraftBoxContract.Presenter {
    ...
    public List<BaseDraftBean> requestCacheData() {
        ...
        } else if (DraftBoxFragment.MY_DRAFT_TYPE_INFO.equals(mRootView.getDraftType())) {
            return mInfoDraftBeanGreenDao.getMultiBasetDraftDataFromCache();
        }
        return null;
    }
    ...
    public void deleteDraft(BaseDraftBean draftBean) {
        if (draftBean instanceof QAPublishBean) {
            ...
            InfoDraftBean deleteData = (InfoDraftBean) draftBean;
            mInfoDraftBeanGreenDao.deleteSingleCache(deleteData);
        }
    }
