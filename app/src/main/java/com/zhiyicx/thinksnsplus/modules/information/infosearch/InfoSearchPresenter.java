...
import static com.zhiyicx.baseproject.base.TSListFragment.DEFAULT_PAGE;
 ...
 * @Description
 */
public class InfoSearchPresenter extends AppBasePresenter<SearchContract.View> implements SearchContract.Presenter {
    ...
    private List<InfoListDataBean> mRecommendInfoList;

    private Subscription subscribeRecommend;
    ...
    public void requestNetData(Long maxId, final boolean isLoadMore) {
        ...
        if (subscribeRecommend != null && !subscribeRecommend.isUnsubscribed()) {
            subscribeRecommend.unsubscribe();
        }
        subscription = mBaseInfoRepository.getInfoListV2("", mRootView.getKeyWords(), maxId, mRootView.getPage(), 0)
...
//                        }
    ...
    public void getRecommendInfo() {
        if (mRecommendInfoList != null) {
            mRootView.onNetResponseSuccess(mRecommendInfoList, false);
        } else {
            subscribeRecommend = mBaseInfoRepository.getInfoListV2("", "", 0, DEFAULT_PAGE, 1)
                    ...
                    .subscribe(new BaseSubscribeForV2<List<InfoListDataBean>>() {
                        ...
                        protected void onSuccess(List<InfoListDataBean> data) {
                            if (!data.isEmpty()) {
                                mRecommendInfoList = data;
                                mRootView.onNetResponseSuccess(mRecommendInfoList, false);
                            }
                        }
                    });
            addSubscrebe(subscribeRecommend);
        }
    }
}
