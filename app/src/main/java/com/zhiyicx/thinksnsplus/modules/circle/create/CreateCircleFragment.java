...
//                .into(mIvHeadIcon);
    ...
    private void initListener() {
        ...
        mCbFree.setOnTouchEventListener(isEnabled -> {
            ...
            if (isPaidCircle && canUpdate && !isManager) {
...
//                .into(mIvHeadIcon);
        ...
        if (!TextUtils.isEmpty(mCircleInfo.getLatitude()) && !TextUtils.isEmpty(mCircleInfo.getLongitude())) {
            ...
            mTvLocation.setText(getString(R.string.create_circle_nolocation));
        }
        ...
        String notice = mCircleInfo.getNotice();
        if (!canUpdate && TextUtils.isEmpty(notice)) {
            notice = "暂无公告";
        }
        mTvNotice.setText(notice);

        String introduce = mCircleInfo.getSummary();
        if (!(canUpdate && !isManager) && TextUtils.isEmpty(introduce)) {
            introduce = "暂无简介";
        }
        mEtCircleIntroduce.setText(introduce);
        ...
        mLlGoneToMember.setVisibility(canUpdate?View.VISIBLE:View.GONE);
        mLlFree.setVisibility(canUpdate && !isManager ? View.VISIBLE : mCircleInfo.getMoney() > 0 ? View.GONE : View.VISIBLE);
        mLlCharge.setVisibility(canUpdate && !isManager ? View.VISIBLE : mCircleInfo.getMoney() > 0 ? View.VISIBLE : View.GONE);
