...
public abstract class InfoListItem implements ItemViewDelegate<BaseListBean> {
    ...
    public boolean isForViewType(BaseListBean item, int position) {
        ...
        if (isInfo) {
            boolean hasImage = ((InfoListDataBean) item).getImages() != null;
            int imageCount = hasImage ? ((InfoListDataBean) item).getImages().size() : 0;
            return !hasImage || imageCount < 3;
        } else {
            return false;
        }
    }
    ...
    public void convert(ViewHolder holder, BaseListBean baseListBean, BaseListBean lastT,
        ...
        imageView.setOnClickListener(v -> holder.getConvertView().performClick());
        // 记录点击过后颜色
