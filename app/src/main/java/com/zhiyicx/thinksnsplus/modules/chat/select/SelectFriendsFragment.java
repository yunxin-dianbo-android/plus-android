...
public class SelectFriendsFragment extends TSListFragment<SelectFriendsContract.Presenter, UserInfoBean>
    ...
    protected RecyclerView mRvSelectResult;
    @BindView(R.id.edit_search_friends)
    AppCompatEditText mEditSearchFriends;
    @BindView(R.id.ll_search)
    LinearLayout mLinearLayout;

    protected List<UserInfoBean> mSelectedList;
    private List<UserInfoBean> mSearchResultList;
    protected SelectedFriendsAdapter mSelectedFriendsAdapter;

    /**
     * 群信息
     */
    protected ChatGroupBean mChatGroupBean;

    /**
     * 是否是删除用户  如果是删除 那么则不用去请求好友列表
     */
    protected boolean mIsDeleteMember;
    ...
    public void onNetResponseSuccess(@NotNull List<UserInfoBean> data, boolean isLoadMore) {
        // 当好友没有时隐藏搜索
        boolean gone = getPage() <= 1 && TextUtils.isEmpty(mEditSearchFriends.getText().toString().trim()) && data.isEmpty();
        mLinearLayout.setVisibility(gone ? View.GONE : View.VISIBLE);
    ...
    public void onCacheResponseSuccess(List<UserInfoBean> data, boolean isLoadMore) {
        boolean gone = isLoadMore && TextUtils.isEmpty(mEditSearchFriends.getText().toString().trim()) && data.isEmpty();
        mLinearLayout.setVisibility(gone ? View.GONE : View.VISIBLE);
        checkUserIsSelected(data);
