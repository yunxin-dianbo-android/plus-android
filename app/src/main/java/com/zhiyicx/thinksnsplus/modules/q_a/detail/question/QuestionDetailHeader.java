...
public class QuestionDetailHeader extends BaseWebLoad implements TagFlowLayout.OnTagClickListener {
    ...
    public QuestionDetailHeader(Activity context, List<RealAdvertListBean> adverts) {
        ...
        mAvatarView = mQuestionHeaderView.findViewById(R.id.iv_user_avatar);
        mTvUserName = mQuestionHeaderView.findViewById(R.id.tv_name);
        mLlUserInfo = mQuestionHeaderView.findViewById(R.id.ll_user_info);
        mTflQuestion = mQuestionHeaderView.findViewById(R.id.tfl_question);
        mTvQuestionTitle = mQuestionHeaderView.findViewById(R.id.tv_question_title);
        mQdContent = mQuestionHeaderView.findViewById(R.id.qd_content);
        mTvQuestionFeedCount = mQuestionHeaderView.findViewById(R.id.tv_question_feed_count);
        mTvQuestionReward = mQuestionHeaderView.findViewById(R.id.tv_question_detail_reward);
        mTvQuestionOnlookAmount = mQuestionHeaderView.findViewById(R.id.tv_question_onlook_amount);
        mTvTopicChangeFollow = mQuestionHeaderView.findViewById(R.id.tv_topic_change_follow);
        mIvRewardType = mQuestionHeaderView.findViewById(R.id.iv_reward_type);
        mTvRewardType = mQuestionHeaderView.findViewById(R.id.tv_reward_type);
        mTvAddAnswer = mQuestionHeaderView.findViewById(R.id.tv_add_answer);
        rewardType = mQuestionHeaderView.findViewById(R.id.ll_reward_type);
        addAnswer = mQuestionHeaderView.findViewById(R.id.ll_add_answer);
        mTvAnswerCount = mQuestionHeaderView.findViewById(R.id.tv_answer_count);
        mTvChangeOrder = mQuestionHeaderView.findViewById(R.id.tv_change_order);
        mLlAnswerInfo = mQuestionHeaderView.findViewById(R.id.ll_answer_info);
        mIvAddAnswer = mQuestionHeaderView.findViewById(R.id.iv_add_answer);
    }
    ...
    public void setDetail(QAListInfoBean qaListInfoBean, double amount, int ratio) {
        ...
        // 用户信息
        boolean isAnonmity = qaListInfoBean.getAnonymity() == 1;
        boolean isSelf = qaListInfoBean.getUser_id() == AppApplication.getMyUserIdWithdefault();
        ImageUtils.loadUserHead(qaListInfoBean.getUser(), mAvatarView, false, !isSelf && isAnonmity);
        RxView.clicks(mLlUserInfo)
                .throttleFirst(JITTER_SPACING_TIME, TimeUnit.SECONDS)
                .subscribe(aVoid -> {
                    if (!isAnonmity) {
                        PersonalCenterFragment.startToPersonalCenter(mLlAnswerInfo.getContext(), mQaListInfoBean.getUser());
                    }
                });
        if (isAnonmity) {
            mTvUserName.setText(!isSelf ? mContext.getResources().getString(R.string.qa_question_answer_anonymity_user)
                    : mQaListInfoBean.getUser().getName() + mContext.getString(R.string.qa_question_answer_anonymity_current_user));
        } else {
            mTvUserName.setText(mQaListInfoBean.getUser().getName());
        }
        ...
        mQdContent.getMdvQuestionContent().setWebChromeClient(mWebChromeClient);
        mQdContent.getMdvQuestionContent().setWebViewClient(mWebViewClient);
    ...
    public MarkdownView getMarkdownView() {
        return mQdContent.getMdvQuestionContent();
    }

    public void destroyedWeb() {
        destryWeb(mQdContent.getMdvQuestionContent());
    }
