...
public class InfoDetailsPresenter extends AppBasePresenter<InfoDetailsConstract.View> implements InfoDetailsConstract.Presenter,
    ...
    @Override
    public void shareInfo(Bitmap bitmap, List<UmengSharePolicyImpl.ShareBean> extraData) {
        ...
        shareContent.setContent(TextUtils.isEmpty(mRootView.getCurrentInfo().getSubject()) ? mContext.getString(R.string
                .share_default, mContext.getString(R.string.app_name)) : mRootView.getCurrentInfo().getSubject());
    ...
    public void onStart(Share share) {
        ...
        int id;
        if (mRootView.getCurrentInfo().getImage() != null) {
            id = mRootView.getCurrentInfo().getImage().getId();
        } else {
            id = RegexUtils.getImageIdFromMarkDown(MarkdownConfig.IMAGE_FORMAT, mRootView.getCurrentInfo().getContent());
        }
        ...
        String content = TextUtils.isEmpty(mRootView.getCurrentInfo().getSubject()) ? mRootView.getCurrentInfo().getContent()
                : mRootView.getCurrentInfo().getSubject();
        ...
        Letter letter = new Letter(mRootView.getCurrentInfo().getTitle(), content, TSEMConstants.BUNDLE_CHAT_MESSAGE_FORWARD_TYPE_INFO);
        letter.setImage(image);
        letter.setId(mRootView.getCurrentInfo().getId() + "");
        ...
        switch (share) {
            case FORWARD:
                ...
                break;
            case LETTER:
                letter.setType(TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_INFO);
    ...
    @Override
    public void handleCollect(boolean isUnCollected, String newsId) {
        ...
        mBaseInfoRepository.handleCollect(isUnCollected, newsId);
    }
    ...
    public void deleteInfo() {
        ...
        if (isManager) {
            observable = mBaseInfoRepository.deleteInfo(String.valueOf(mRootView.getNewsId()));
        } else {
            observable = mBaseInfoRepository.deleteInfo(String.valueOf(mRootView.getCurrentInfo().getCategory().getId()),
        ...
        Subscription subscription = observable
                .subscribe(new BaseSubscribeForV2<BaseJsonV2<Object>>() {
                    ...
                    protected void onSuccess(BaseJsonV2<Object> data) {
                        mInfoListBeanGreenDao.deleteInfo(mRootView.getCurrentInfo());
                        mRootView.deleteInfo(false, true, (data == null || data.getMessage() == null || data.getMessage().isEmpty()) ? "删除成功" :
                                data.getMessage().get(0));
                    }
    ...
    @Override
    public void handleFollowUser(UserInfoBean authorUserInfoBean) {
        mUserInfoRepository.handleFollow(authorUserInfoBean);
        mInfoListBeanGreenDao.updateInfo(mRootView.getCurrentInfo());
        mRootView.upDateFollowFansState(authorUserInfoBean.isFollower());
    }

    @Override
    public void updatePublishUserFromLocal() {
        if (mRootView.getCurrentInfo() == null || mRootView.getCurrentInfo().getAuthorUserInfoBean() == null || mRootView.getCurrentInfo()
                .getAuthorUserInfoBean().getUser_id() == null) {
            return;
        }
        mRootView.getCurrentInfo().setAuthorUserInfoBean(mUserInfoBeanGreenDao.getSingleDataFromCache(mRootView.getCurrentInfo()
                .getAuthorUserInfoBean().getUser_id()));
        mInfoListBeanGreenDao.updateInfo(mRootView.getCurrentInfo());
        mRootView.upDateFollowFansState(mRootView.getCurrentInfo()
                .getAuthorUserInfoBean().isFollower());
    }
