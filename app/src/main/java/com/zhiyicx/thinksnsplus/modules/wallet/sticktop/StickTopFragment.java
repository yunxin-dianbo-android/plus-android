...
public class StickTopFragment extends TSFragment<StickTopContract.Presenter> implements StickTopContract.View {
    ...
    public static final int PINNED = 1994;
    ...
    public void onFailure(String message, int code) {
        mErrorCode = code;
        mIlvPassword.setErrorTip(message);
    }
    ...
    private void initListener() {
        ...
        RxView.clicks(mBtTop)
                ...
                .compose(this.bindToLifecycle())
                .subscribe(aVoid -> {
                    if (mPresenter.usePayPassword() && mBlance >= mCurrentDays * mInputMoney) {
                        mIlvPassword.setPayNote(new InputPasswordView.PayNote(null, child_id, parent_id));
                        showInputPsdView(true);
                    } else {
                        if (!mIsSourceTop) {
                            mPresenter.stickTop(parent_id, child_id, null);
                        } else {
                            mPresenter.stickTop(parent_id, null);
                        }
                    }
                });
    }
    ...
    public static void startSticTopActivity(Context context, String type, Long parentId, Long childId, boolean sourceIsMine) {
        ...
        intent.putExtras(bundle);
        if (sourceIsMine && context instanceof Activity) {
            ((Activity) context).startActivityForResult(intent, StickTopFragment.PINNED);
        } else {
            context.startActivity(intent);
        }
    }

    public static void startSticTopActivity(android.support.v4.app.Fragment context, String type, Long parentId, Long childId, boolean sourceIsMine) {
        Bundle bundle = new Bundle();
        bundle.putString(StickTopFragment.TYPE, type);
        bundle.putLong(StickTopFragment.PARENT_ID, parentId);
        if (childId != null) {
            bundle.putLong(StickTopFragment.CHILD_ID, childId);
        }
        bundle.putBoolean(StickTopFragment.SOURCE_IS_MINE, sourceIsMine);
        Intent intent = new Intent(context.getContext(), StickTopActivity.class);
        intent.putExtras(bundle);
        if (sourceIsMine) {
            context.startActivityForResult(intent, StickTopFragment.PINNED);
        } else {
            context.startActivity(intent);
        }
    }
