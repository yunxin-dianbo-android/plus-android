...
public class ExpertSearchPresenter extends AppBasePresenter<ExpertSearchContract.View>
    ...
    @Override
    public void requestNetData(Long maxId, int topicId, boolean isLoadMore) {
        Subscription subscribe = mBaseQARepository.getTopicExperts(maxId, topicId)
    ...
    @Override
    public void requestNetData(int size, String topicIds, String keyword, boolean isLoadMore) {
        // mRepository.getExpertList(size, topic_ids, keyword)
        Observable<List<ExpertBean>> observable;
        if (!TextUtils.isEmpty(topicIds)) {
            observable = mBaseQARepository.getExpertList(size, topicIds, keyword);
        } else {
            observable = mUserInfoRepository.searchUserInfo(null, keyword, null, null, null)
                    .flatMap((Func1<List<UserInfoBean>, Observable<List<ExpertBean>>>) userInfoBeen -> {
                        Gson gson = new Gson();
                        Type needType = new TypeToken<List<ExpertBean>>() {
                        }.getType();
