        ...
        switch (messageItemBean.getConversation().getType()) {
            case Chat:
                ...
                if (singleChatUserinfo == null) {
                    long userID = 0;
                    try {
                        userID = Long.parseLong(messageItemBean.getEmKey());
                    } catch (NumberFormatException ignored) {
                    }

                    singleChatUserinfo = DefaultUserInfoConfig.getDefaultDeletUserInfo(holder.getConvertView().getContext(), userID);
                }
            ...
            switch (message.getType()) {
                case TXT:
                    ...
                    switch (letter) {
                        case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_DYNAMIC:
                            type = mContext.getString(R.string.chat_type_dynamic);
                            break;
                        case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_INFO:
                            type = mContext.getString(R.string.chat_type_info);
                            break;
                        case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_CIRCLE:
                            type = mContext.getString(R.string.chat_type_circle);
                            break;
                        case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_POST:
                            type = mContext.getString(R.string.chat_type_post);
                            break;
                        case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_QUESTION:
                            type = mContext.getString(R.string.chat_type_question);
                            break;
                        case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_ANSWER:
                            type = mContext.getString(R.string.chat_type_answer);
                            break;
                        default:
                            type = ((EMTextMessageBody) message.getBody()).getMessage();
                    }
                ...
                case IMAGE:
                    ...
                    if (message.getBooleanAttribute(TSEMConstants.BUNDLE_LOCATION_IMAGE, false)) {
                        ...
                        type = mContext.getString(R.string.chat_type_image);
                        content = messageItemBean.getConversation().isGroup() ? lastUserName
                                + type : type;
                    }
                    break;
...
//                YoYo.with(Techniques.Tada).duration(500).delay(100).playOn(layout.findViewById(R.id.trash));
        ...
        RxView.longClicks(holder.getView(R.id.rl_left))
                ...
                .subscribe(aVoid -> {
                    ...
                    // 小助手不可以被删除
                    boolean isCantDelete = messageItemBean.getConversation().getType() == EMConversation.EMConversationType.Chat && messageItemBean
                            .getUserInfo() != null && mPresenter.checkUserIsImHelper(messageItemBean.getUserInfo()
                            .getUser_id());
