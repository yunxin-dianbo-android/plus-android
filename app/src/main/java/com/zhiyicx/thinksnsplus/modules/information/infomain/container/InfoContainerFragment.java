...
public class InfoContainerFragment extends TSViewPagerFragment<InfoMainContract.InfoContainerPresenter>
    ...
    public void setUserCertificationInfo(UserCertificationInfo userCertificationInfo) {
        ...
        if (userCertificationInfo.getStatus() == UserCertificationInfo.CertifyStatusEnum.PASS.value || !publishInfoConfig.getContribute().hasVerified()) {
            if (mPresenter.isNeedPayTip() && (publishInfoConfig != null
                    && publishInfoConfig.getContribute().hasPay())) {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (data != null) {
            InfoTypeBean infoTypeBean = data.getBundleExtra(SUBSCRIBE_EXTRA).getParcelable(SUBSCRIBE_EXTRA);
            List<InfoTypeCatesBean> catesBeanList = infoTypeBean.getMy_cates();
            List<InfoTypeCatesBean> myCates = mInfoTypeBean.getMy_cates();
            if (catesBeanList.toString().equals(myCates.toString())) {
                return;
            }
            mTitle.clear();
            mFragmentList.clear();
    ...
    private void initPopWindow() {
        ...
        if (mPayAlertPopWindow == null) {
            mPayAlertPopWindow = ActionPopupWindow.builder()
                    ...
                    .desStr(getString(R.string.info_publish_hint_pay, mPresenter.getSystemConfigBean().getNewsContribute().getPay_contribute(), mPresenter
                            .getGoldName()))
                    .bottomStr(getString(R.string.cancel))
                    .isOutsideTouch(true)
                    .isFocus(true)
