...
public class ManuscriptListFragment extends TSListFragment<ManuscriptListContract.Presenter, InfoListDataBean>
    ...
    protected RecyclerView.Adapter getAdapter() {
        ...
        adapter.addItemViewDelegate(new InfoListItem(!getMyInfoType().endsWith(MY_INFO_TYPE_DONE)) {
            ...
            @Override
            public void itemLongClick(int position, ImageView imageView, TextView title, InfoListDataBean realData) {
                if (MY_INFO_TYPE_ERROR.equals(getMyInfoType())) {
                    showDeleteTipPopupWindow(getString(R.string.delete_info), () -> mPresenter.deleteInfo(realData), true);
                }
            }
        });
    ...
    private void toDetail(ImageView imageView, TextView title, InfoListDataBean realData) {
        if (TouristConfig.INFO_DETAIL_CAN_LOOK || !mPresenter.handleTouristControl()) {
            ...
            if (getMyInfoType().equals(MY_INFO_TYPE_ERROR)) {
                ...
                EditeInfoDetailActivity.startEditeInfoDetailActivity(mActivity,infoPublishBean);
                return;
            }
