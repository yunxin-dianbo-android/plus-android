 ...
 */
public abstract class BaseMessageConversationFragment<P extends MessageConversationContract.Presenter<T>,T extends MessageItemBeanV2>
        extends TSListFragment<P, T>
        implements MessageConversationContract.View<P,T>, MessageAdapterV2.OnSwipeItemClickListener,
...
//            mPresenter.refreshConversationReadMessage();
     ...
     */
    protected void toChatV2(MessageItemBeanV2 messageItemBean, int position) {
    ...
    public void onTSEMessageEventEventBus(TSEMessageEvent event) {
        ...
        switch (body.action()) {
            ...
            case TSEMConstants.TS_ATTR_GROUP_DISBAND:
                ...
                NotificationUtil.showTextNotification(mActivity, getString(R.string.group_dissolution, groupName));
                EventBus.getDefault().post(new String[]{groupId, getString(R.string.group_dissolution, groupName)}, EVENT_IM_DELETE_QUIT);
                break;
            case TSEMConstants.TS_ATTR_GROUP_LAYOFF:
                if (TextUtils.isEmpty(groupId)) {
                    return;
                }
                NotificationUtil.showTextNotification(mActivity, getString(R.string.group_removed, groupName));
                EventBus.getDefault().post(new String[]{groupId, getString(R.string.group_removed, groupName)}, EVENT_IM_DELETE_QUIT);
