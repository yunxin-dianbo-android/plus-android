...
import com.zhiyicx.thinksnsplus.modules.chat.item.presenter.TSChatAnswerPresenter;
import com.zhiyicx.thinksnsplus.modules.chat.item.presenter.TSChatCallPresneter;
import com.zhiyicx.thinksnsplus.modules.chat.item.presenter.TSChatCirclePresenter;
import com.zhiyicx.thinksnsplus.modules.chat.item.presenter.TSChatFeedPresenter;
import com.zhiyicx.thinksnsplus.modules.chat.item.presenter.TSChatFilePresenter;
import com.zhiyicx.thinksnsplus.modules.chat.item.presenter.TSChatInfoPresenter;
...
import com.zhiyicx.thinksnsplus.modules.chat.item.presenter.TSChatPostPresenter;
import com.zhiyicx.thinksnsplus.modules.chat.item.presenter.TSChatQuestionPresenter;
...
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_RECV_EXPRESSION;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_RECV_FILE;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_RECV_IMAGE;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_RECV_LETTER_ANSWER;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_RECV_LETTER_CIRCLE;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_RECV_LETTER_FEED;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_RECV_LETTER_INFO;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_RECV_LETTER_POST;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_RECV_LETTER_QUESTION;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_RECV_LOCATION;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_RECV_TXT;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_RECV_VIDEO;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_RECV_VOICE;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_SENT_EXPRESSION;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_SENT_FILE;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_SENT_IMAGE;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_SENT_LETTER_ANSWER;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_SENT_LETTER_CIRCLE;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_SENT_LETTER_FEED;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_SENT_LETTER_INFO;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_SENT_LETTER_POST;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_SENT_LETTER_QUESTION;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_SENT_LOCATION;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_SENT_TXT;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_SENT_VIDEO;
import static com.hyphenate.easeui.adapter.EaseMessageAdapter.MESSAGE_TYPE_SENT_VOICE;
...
public class ChatFragment extends TSEaseChatFragment<ChatContract.Presenter>
        implements TSEaseChatFragment.EaseChatFragmentHelper, EaseChatRowPresenter.MessageErrorListener,
        ChatContract.View, EaseChatRow.OnTipMsgClickListener, TSChatCirclePresenter.OnCircleClickListener,
        TSChatPostPresenter.OnPostClickListener {
    ...
    @Override
    public void handleSendMessageError(EMMessage message, int code, String error) {
        if (code == 602) {
            EventBus.getDefault().post(new String[]{message.getTo(), getString(R.string.group_error)}, EVENT_IM_DELETE_QUIT);
        }
    }
    ...
    protected void setUpView() {
        ...
        if (currentLoginUserName == null) {
            currentLoginUserName = mPresenter.getCurrenLoginUserName(AppApplication.getMyUserIdWithdefault());
        }
        ...
        // showComment forward message if the message is not null
        ...
        if (fragmentArgs != null) {
            String type = fragmentArgs.getString(TSEMConstants.BUNDLE_CHAT_MESSAGE_TYPE);
            if (TSEMConstants.BUNDLE_CHAT_MESSAGE_TYPE_LOCATION.equals(type)) {
                String image, latitude, longitude, address, name;
                image = fragmentArgs.getString(TSEMConstants.BUNDLE_LOCATION_IMAGE);
                latitude = fragmentArgs.getString(TSEMConstants.BUNDLE_LOCATION_LATITUDE);
                longitude = fragmentArgs.getString(TSEMConstants.BUNDLE_LOCATION_LONGITUDE);
                address = fragmentArgs.getString(TSEMConstants.BUNDLE_LOCATION_ADDRESS);
                name = fragmentArgs.getString(TSEMConstants.BUNDLE_LOCATION_TITLE);
                ...
                if (!TextUtils.isEmpty(image)) {
                    ...
                    EMMessage message = EMMessage.createImageSendMessage(image, isNeedOrinImage, toChatUsername);
                    message.setAttribute(TSEMConstants.BUNDLE_LOCATION_LATITUDE, latitude + "");
                    message.setAttribute(TSEMConstants.BUNDLE_LOCATION_LONGITUDE, longitude + "");
                    message.setAttribute(TSEMConstants.BUNDLE_LOCATION_ADDRESS, address);
                    message.setAttribute(TSEMConstants.BUNDLE_LOCATION_TITLE, name);
                    message.setAttribute(TSEMConstants.BUNDLE_LOCATION_IMAGE, true);

                    sendMessage(message);
                }
            } else if (TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER.equals(type)) {
                Letter letter = fragmentArgs.getParcelable(TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER);
                if (letter == null) {
                    return;
                }
                EMMessage message = EMMessage.createTxtSendMessage(letter.getContent(), toChatUsername);

                message.setAttribute(TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER, letter.getType());
                message.setAttribute(TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_IMAGE, letter.getImage());
                message.setAttribute(TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_NAME, letter.getName());
                message.setAttribute(TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_CONTENT, letter.getContent());
                sendMessage(message);
        ...
        switch (itemId) {
            ...
            case ITEM_LOCATION_TS:
                ...
                mRxPermissions.request(Manifest.permission.ACCESS_FINE_LOCATION)
                        .subscribe(permission -> {
                            if (permission) {
                                SendLocationActivity.startSendLocationActivity(this, mActivity, mToolbarCenter.getText().toString());
                            } else {
    ...
    @Override
    public void onCircleClick(Long id) {
        CircleDetailActivity.startCircleDetailActivity(mActivity, id);
//        mPresenter.getCircleInfo(id, null);
    }

    @Override
    public void onPostClick(Long circleId, Long postId) {
        CirclePostDetailActivity.startCirclePostDetailActivity(mActivity, circleId, postId, false);
    }

    @Override
    public void gotoCircleDetail(CircleInfo circleInfo, Long postId) {
        ...
        if (postId == null) {
            ...
            CirclePostDetailActivity.startCirclePostDetailActivity(getContext(), circleInfo.getId(), postId);
        }
    }
        ...
        for (EMMessage message : messages) {
            ...
            if (username.equals(toChatUsername) || message.getTo().equals(toChatUsername)
                ...
                if (message.getBody() instanceof EMTextMessageBody) {
                    String letter = message.getStringAttribute(TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER, "");
                    switch (letter) {
                        case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_DYNAMIC:
                            content = mActivity.getString(R.string.chat_type_dynamic);
                            break;
                        case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_INFO:
                            content = mActivity.getString(R.string.chat_type_info);
                            break;
                        case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_CIRCLE:
                            content = mActivity.getString(R.string.chat_type_circle);
                            break;
                        case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_POST:
                            content = mActivity.getString(R.string.chat_type_post);
                            break;
                        case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_QUESTION:
                            content = mActivity.getString(R.string.chat_type_question);
                            break;
                        case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_ANSWER:
                            content = mActivity.getString(R.string.chat_type_answer);
                            break;
                        default:
                            content = ((EMTextMessageBody) message.getBody()).getMessage();
                    }
                } else if (message.getBody() instanceof EMImageMessageBody) {
                    if (message.getBooleanAttribute(TSEMConstants.BUNDLE_LOCATION_IMAGE, false)) {
                        content = mActivity.getString(R.string.chat_type_location);
                    } else {
                        content = mActivity.getString(R.string.chat_type_image);
                    }
                } else if (message.getBody() instanceof EMVoiceMessageBody) {
                    content = getString(R.string.chat_type_voice);
                } else if (message.getBody() instanceof EMLocationMessageBody) {
                    content = getString(R.string.chat_type_location);
                }

                content = mPresenter.getUserName(message.getFrom()) + ":" + content;
                jpushMessageBean.setMessage(content);
                NotificationUtil.showChatNotifyMessageExceptCurrentConversation(AppApplication.getContext(), jpushMessageBean,
            ...
            if (message.getType() == EMMessage.Type.TXT) {
                if (message.getType() == EMMessage.Type.TXT) {
                    ...
                    //messagee recall
                        ...
                        switch (letter) {
                            case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_DYNAMIC:
                                return message.direct() == EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_LETTER_FEED : MESSAGE_TYPE_SENT_LETTER_FEED;
                            case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_INFO:
                                return message.direct() == EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_LETTER_INFO : MESSAGE_TYPE_SENT_LETTER_INFO;
                            case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_CIRCLE:
                                return message.direct() == EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_LETTER_CIRCLE :
                                        MESSAGE_TYPE_SENT_LETTER_CIRCLE;
                            case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_POST:
                                return message.direct() == EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_LETTER_POST : MESSAGE_TYPE_SENT_LETTER_POST;
                            case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_QUESTION:
                                return message.direct() == EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_LETTER_QUESTION :
                                        MESSAGE_TYPE_SENT_LETTER_QUESTION;
                            case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_ANSWER:
                                return message.direct() == EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_LETTER_ANSWER :
                                        MESSAGE_TYPE_SENT_LETTER_ANSWER;
                            default:
        ...
        public EaseChatRowPresenter getCustomChatRow(EMMessage message, int position, BaseAdapter adapter, ChatUserInfoBean userInfoBean) {
            if (message.getType() == EMMessage.Type.TXT) {
                ...
                if (message.getBooleanAttribute(EaseConstant.MESSAGE_ATTR_IS_VOICE_CALL, false) ||
                    ...
                    switch (letter) {
                        case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_DYNAMIC:
                            presenter = new TSChatFeedPresenter();
                            break;
                        case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_INFO:
                            presenter = new TSChatInfoPresenter();
                            break;
                        case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_CIRCLE:
                            presenter = new TSChatCirclePresenter(ChatFragment.this);
                            break;
                        case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_POST:
                            presenter = new TSChatPostPresenter(ChatFragment.this);
                            break;
                        case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_QUESTION:
                            presenter = new TSChatQuestionPresenter();
                            break;
                        case TSEMConstants.BUNDLE_CHAT_MESSAGE_LETTER_TYPE_ANSWER:
                            presenter = new TSChatAnswerPresenter();
                            break;
                        default:
                    }
                }
