    ...
    public void onNetResponseSuccess(@NotNull List<BaseListBean> data, boolean isLoadMore) {
        try {// 添加广告
            if (!data.isEmpty() && mListAdvert != null && mListAdvert.size() >= getPage()) {
                ...
                data.add(rand.nextInt(data.size() - 1) + 1, DynamicListAdvert.advert2Info(advert, maxId));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    protected MultiItemTypeAdapter getAdapter() {
        ...
        adapter.addItemViewDelegate(new InfoListItem(false) {
            ...
            public void itemClick(int position, ImageView imageView, TextView title, InfoListDataBean realData) {
                toInfoDetail(imageView, title, realData);
            }
        });
...
//        onCacheResponseSuccess(requestCacheData(mMaxId, false), false);
    ...
    private void toInfoDetail(ImageView imageView, TextView title, InfoListDataBean realData) {
