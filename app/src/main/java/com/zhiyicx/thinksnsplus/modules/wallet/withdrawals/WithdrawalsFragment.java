...
public class WithdrawalsFragment extends TSFragment<WithDrawalsConstract.Presenter> implements WithDrawalsConstract.View {
    ...
    protected void initData() {
        ...
        if (getArguments() != null && mSystemConfigBean
                .getWallet()
                .getCash() != null) {
            mTvWithdrawDec.setText(String.format(getString(R.string.min_withdraw_money_limit), PayConfig.realCurrencyFen2Yuan(mSystemConfigBean
                    .getWallet()
                    .getCash().getMinAmount()), getString(R.string.yuan)));
        }
    ...
    private void initWithDrawalsStylePop() {
        ...
        if (mSystemConfigBean.getWallet().getCash() != null && mSystemConfigBean.getWallet().getCash().getTypes() != null) {
            cash_types.addAll(Arrays.asList(mSystemConfigBean.getWallet().getCash().getTypes()));
        }
        ...
        mActionPopupWindow = ActionPopupWindow.builder()
                .item2Str(cash_types.contains(TSPayClient.CHANNEL_ALIPAY) ? getString(R.string.choose_withdrawals_style_formart, getString(R
                        .string.alipay)) : "")
                .item3Str(cash_types.contains(TSPayClient.CHANNEL_WXPAY) || cash_types.contains(TSPayClient.CHANNEL_WX) ? getString(R.string
                        .choose_withdrawals_style_formart, getString(R
                        .string.wxpay)) : "")
    ...
    public void onDestroyView() {
        ...
        dismissPop(mWithdrawalsInstructionsPopupWindow);
        super.onDestroyView();
    }
}
