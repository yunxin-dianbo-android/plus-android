 ...
 */
@FragmentScoped
public class PersonalCenterPresenter extends AppListPresenterForDownload<PersonalCenterContract.View> implements
    ...
    @Override
    public void requestNetData(Long maxId, final boolean isLoadMore, final long userId) {
        if (AppApplication.getmCurrentLoginAuth() == null) {
            return;
        }
        Subscription subscription =
                mBaseDynamicRepository.getDynamicListForSomeone(userId, maxId, mRootView.getDynamicType())
                        ...
                        .map(dynamicDetailBeanV2s -> {
                            ...
                            for (DynamicDetailBeanV2 detailBeanV2 : dynamicDetailBeanV2s) {
                                if (detailBeanV2.getUser_id() == userId) {
                        ...
                        .map(listBaseJson -> {
                            ...
                            if (!isLoadMore && AppApplication.getmCurrentLoginAuth().getUser_id() == userId) {
...
//        mRootView.onCacheResponseSuccess(null, isLoadMore);
    ...
    public void uploadUserCover(String filePath, UserInfoBean userInfoBean) {
        Subscription subscription = mIUploadRepository.uploadUserBg(filePath)
                .subscribe(new BaseSubscribeForV2<UploadTaskResult>() {
                    ...
                    protected void onSuccess(UploadTaskResult data) {
                        ...
                        userInfoBean.setCover(new Avatar(filePath));
                        // 更新用户数据库
    ...
    @Override
    public void setCurrentUserInfo(Object userId) {
        if (AppApplication.getmCurrentLoginAuth() == null || userId == null) {
            return;
        }
        List<Object> userids = new ArrayList<>();
        userids.add(userId);
        Observable<List<UserTagBean>> userTags = userId instanceof String ? null : mUserInfoRepository.getUserTags((long) userId);
        boolean isMine = !(userId instanceof String) && AppApplication.getMyUserIdWithdefault() == (long) userId;

        if (userTags == null) {
            mUserInfoRepository.getUserInfo(userids)
                    .flatMap((Func1<List<UserInfoBean>, Observable<UserInfoBean>>) userInfoBeans -> Observable.just(userInfoBeans.get(0)))
                    .subscribe(new BaseSubscribeForV2<UserInfoBean>() {
                        @Override
                        protected void onSuccess(UserInfoBean userInfoBean) {
                            if (userInfoBean.getUser_id() == null || userInfoBean.getUser_id() == 0) {
                                mRootView.loadAllError();
                                return;
                            }
                            mUserInfoRepository.getUserTags(userInfoBean.getUser_id())
                                    .subscribe(new BaseSubscribeForV2<List<UserTagBean>>() {
                                        @Override
                                        protected void onSuccess(List<UserTagBean> data) {
                                            userInfoBean.setTags(data);
                                            mUserInfoBeanGreenDao.insertOrReplace(userInfoBean);
                                            mInterfaceNum++;
                                            mRootView.setHeaderInfo(userInfoBean);
                                            requestNetData(DEFAULT_PAGE_MAX_ID, false, userInfoBean.getUser_id());
                                            allready();
                                        }
                                    });
                        }
            ...
            Subscription subscription = Observable.zip(userTags,
                    isMine ? mUserInfoRepository.getCurrentLoginUserInfo()
                            : mUserInfoRepository.getUserInfo(userids)
                    ...
                    .subscribe(new BaseSubscribeForV2<UserInfoBean>() {
                        ...
                        protected void onSuccess(UserInfoBean data) {
                            ...
                            requestNetData(DEFAULT_PAGE_MAX_ID, false, data.getUser_id());
                            allready();
                        }
    ...
    public void reSendDynamic(int position) {
        ...
        // feed_mark作为参数
        if (mRootView.getListDatas().get(position).getFeed_mark() != null) {
    ...
    @Override
    public void payNote(int dynamicPosition, int imagePosition, int note, boolean isImage, String psd) {
    ...
    public void deleteDynamic(DynamicDetailBeanV2 dynamicBean) {
        ...
        Subscription subscription = Observable.just(dynamicBean)
                .observeOn(Schedulers.io())
