...
public class BaseCircleRepository implements IBaseCircleRepository {
    ...
    public Observable<List<CircleInfo>> getAllCircle(Integer limit, Integer offet, String keyword
            , Integer category_id, String id) {
        return mCircleClient.getAllCircle(limit, offet, keyword, category_id, id)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread());
    }
    ...
    @Override
    public void dealExcellent(long postId) {
        Observable.just(postId)
                ...
                .subscribe(id -> {
                    ...
                    HashMap<String, Object> params = new HashMap<>();
                    // 后台处理
                    backgroundRequestTaskBean = new BackgroundRequestTaskBean(BackgroundTaskRequestMethodConfig.PUT, params);
                    backgroundRequestTaskBean.setPath(
                            String.format(ApiConfig.APP_PATH_EXCELLENT_POST_FORMAT, String.valueOf(postId)));
                    BackgroundTaskManager.getInstance(mContext).addBackgroundRequestTask(backgroundRequestTaskBean);
    ...
    @Override
    public Observable<BaseJsonV2<Object>> dealCircleJoinOrExit(CircleInfo circleInfo,String psd) {
        ...
        if (isJoined) {
            ...
            observable = mCircleClient.joinCircle(circleInfo.getId(),psd);
        }
...
//        return null;
    ...
    @Override
    public Observable<List<CirclePostListBean>> getPrePostListFromCircle(Long circleId) {
        return dealWithPostList(mCircleClient.getPrePostListFromCircle(circleId))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread());
    }
