...
import static com.zhiyicx.baseproject.config.ApiConfig.APP_PATH_CKEAR_USER_APPEND_READ_MESSAGE;
...
import static com.zhiyicx.baseproject.config.ApiConfig.APP_PATH_GET_CURRENT_USER_PERMISSION;
...
public interface UserInfoClient {
     ...
     * 修改用户资料
     *
     * @param userInfoTaskParams 用户需要修改哪那些信息不确定
     */
    @PATCH(APP_PATH_CHANGE_USER_INFO)
    Observable<Object> changeUserInfo(@Body UpdateUserInfoTaskParams userInfoTaskParams);
     ...
     * @param fetch_by 可选，获取数据的方式，默认是 id 已常规方式进行获取，允许值：username 使用 name 字段进行按照用户名获取、id 使用 id 字段按照用户 ID 进行获取。
     * @return
     */
    ...
    Observable<List<UserInfoBean>> getBatchSpecifiedUserInfo(@Query("id") String user_ids, @Query("name") String name, @Query("since") Integer
            since, @Query("order") String order, @Query("limit") Integer limit,@Query("fetch_by") String fetch_by);
     ...
     * 获取用户收到的@
     *
     * @param index 用来翻页数据体记录 id
     * @param limit 返回数据条数 默认 20 条
     * @param direction 可选，以 index 标记开始的数据顺序，默认 desc，允许值：asc/desc。
     * @return
     */
    @GET(ApiConfig.APP_PATH_GET_MY_ATMESSAGES)
    Observable<List<AtMeaasgeBean>> getAtMessages(@Query("index") int index,
                                                  @Query("limit") Integer limit, @Query("direction") String direction);
    ...
    Observable<List<AtMeaasgeBean>> getAllComments(@Query("limit")Integer limit, @Query("index")int index,
                                                   @Query("direction")String direction,@Query("author")Long author,
                                                   @Query("for_user") Long forUser,@Query("for_type")String forType,
                                                   @Query("resourceable_id")String resourceableId,@Query("resourceable_type")String resourceableType,
                                                   @Query("id")String id);
     ...
     * 通知列表
     *
     * @param type 必须，请传递通知类型
     * @param page 可选，分页页码，默认 1
     * @return
     * @see {https://slimkit.github.io/plus/api-v2/user/notifications.html#%E8%8E%B7%E5%8F%96%E9%80%9A%E7%9F%A5%E7%BB%9F%E8%AE%A1}
     */
    @GET(ApiConfig.APP_PATH_GET_NOTIFICATION_LIST)
    Observable<UserNotifyMsgBean> getNotificationList(@Query("type") String type, @Query
            ("page") Integer page);
    ...
    Observable<Object> makeNotificationReaded(@Query("type") String type);
    ...
    @PATCH(ApiConfig.APP_PATH_CHANGE_USER_INFO)
    Observable<Object> updateUserInfo(@Body UpdateUserInfoTaskParams params);
    ...
    Observable<Object> rewardUser(@Path("user_id") long user_id, @Field("amount") long amount, @Field("password") String psd);
     ...
     * @return
     */
    @PATCH(APP_PATH_CKEAR_USER_APPEND_READ_MESSAGE)
     ...
     * @return
     */
    @GET(APP_PATH_GET_CURRENT_USER_PERMISSION)
