...
public class BaseMessageRepository implements IBaseMessageRepository {
    ...
    public Observable<List<MessageItemBeanV2>> getConversationList(int userId) {
        ...
        return Observable.just(conversations)
                ...
                .concatMap(stringEMConversationMap -> {
                        ...
                        if (itemBeanV2.getConversation().getType() == EMConversation.EMConversationType.Chat) {
                            ...
                            if (!itemBeanV2.getEmKey().equals("admin")) {
                                ...
                                if (itemBeanV2.getUserInfo() == null) {
                                    try {
                                        users.add(Long.parseLong(itemBeanV2.getEmKey()));
                                ...
                                if ("admin".equals(message.getFrom())) {
                                    ...
                                    try {
                                        ...
                                        if (mUserInfoBeanGreenDao.getSingleDataFromCache(userId) == null) {
                                            users.add(Long.parseLong(itemBeanV2.getConversation().getLastMessage().getFrom()));
                                        }
                                    } catch (Exception ignore) {
...
//                                                                    }
//
//                                                                    return Observable.timer(RETRY_DELAY_TIME,
//                                                                            TimeUnit.SECONDS);
//                                                                }
...
//                                        })
     ...
     */
    @Override
    public Observable<List<ChatGroupBeanForGroupList>> getGroupInfoOnlyGroupFace(String ids) {
        return mClient.getGroupInfoOnlyGroupFace(ids)
                .subscribeOn(Schedulers.io())
                .observeOn(Schedulers.io())
                .flatMap((Func1<List<ChatGroupBeanForGroupList>, Observable<List<ChatGroupBeanForGroupList>>>) chatGroupBeanForGroupLists -> {
                    for (ChatGroupBeanForGroupList chatGroupBeanForGroupList : chatGroupBeanForGroupLists) {
                        chatGroupBeanForGroupList.handleData();
                    }
                    return Observable.just(chatGroupBeanForGroupLists);
                });
    }
    ...
    @Override
    public void saveChatGoupForGroupList(List<ChatGroupBeanForGroupList> groupBeans) {
        mChatGroupBeanGreenDao.saveMultiDataForGroupList(groupBeans);
    }
                ...
                .flatMap(list1 -> {
                    ...
                    for (ChatItemBean chatItemBean : list1) {
                        if ("admin".equals(chatItemBean.getMessage().getFrom())) {
                            ...
                            try {
                                Long userId = Long.parseLong(chatItemBean.getMessage().getFrom());
                                users.add(userId);
                            } catch (Exception ignore) {
