...
import static com.zhiyicx.baseproject.config.ApiConfig.APP_PATH_INFO_MANAGER_DELETE;
...
public interface InfoMainClient {
     ...
     * @param cate_id 分类id
     * @param max_id 数据翻页标识
     * @param limit 数据返回条数 默认为20
     * @param page
     * @param key 搜索关键字
     * @param isRecommend 推荐筛选 =1为筛选推荐资讯列表
     * @return
     */
     ...
     * @param key 搜索关键字
     * @param isRecommend 推荐筛选 =1为筛选推荐资讯列表
     * @param id 需要按照 ID 获取的资讯 ID，多个使用半角 , 进行分割
     * @return
     */
    @GET(APP_PATH_INFO_LIST_V2)
    Observable<List<InfoListDataBean>> getInfoList(@Query("cate_id") String cate_id,
                                                     @Query("after") Long max_id,
                                                     @Query("limit") Integer limit,
                                                     @Query("key") String key,
                                                     @Query("recommend") Integer isRecommend,
                                                     @Query("id") String id);
    ...
    @DELETE(APP_PATH_INFO_MANAGER_DELETE)
    Observable<BaseJsonV2<Object>> deleteInfo(@Path("id") String id);
    ...
    Observable<BaseJsonV2<Integer>> stickTopInfo(@Path("news_id") Long news_id, @Field("amount") long amount,
    ...
    Observable<BaseJsonV2<Integer>> stickTopInfoComment(@Path("news_id") Long news_id, @Path("comment_id") Long comment_id, @Field("amount") long
            amount, @Field("day") int day,@Field("password") String psd);
    ...
    Observable<Object> rewardInfo(@Path("news_id") long news_id, @Field("amount") long amount, @Field("password") String psd);
