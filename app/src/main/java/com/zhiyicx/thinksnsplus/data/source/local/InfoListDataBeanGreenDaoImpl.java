...
public class InfoListDataBeanGreenDaoImpl extends CommonCacheImpl<InfoListDataBean> {
    ...
    public long saveSingleData(InfoListDataBean singleData) {
        return getWDaoSession().getInfoListDataBeanDao().insertOrReplace(singleData);
    }

    @Override
    public void saveMultiData(List<InfoListDataBean> multiData) {
        getWDaoSession().getInfoListDataBeanDao().insertOrReplaceInTx(multiData);
    }
    ...
    public void deleteSingleCache(InfoListDataBean dta) {
        getWDaoSession().getInfoListDataBeanDao().delete(dta);
    }

    @Override
    public void updateSingleData(InfoListDataBean newData) {
        getWDaoSession().getInfoListDataBeanDao().insertOrReplace(newData);
    }
    ...
    public boolean isDiged(int news_id) {
        try {
            return getRDaoSession().getInfoListDataBeanDao().queryBuilder().where(InfoListDataBeanDao.Properties.Id.eq(news_id)).build().list().get(0).getHas_like();
        } catch (Exception e) {
            return false;
        }
    ...
    public boolean isCollected(int news_id) {
        try {
            return getRDaoSession().getInfoListDataBeanDao().queryBuilder().where(InfoListDataBeanDao.Properties.Id.eq(news_id)).build().list()
    ...
    public List<InfoListDataBean> getInfoByType(Long cate_id) {
        try {
            return getRDaoSession().getInfoListDataBeanDao().queryBuilder().where(InfoListDataBeanDao.Properties.Info_type.eq(cate_id)).build().list();
        } catch (Exception e) {
