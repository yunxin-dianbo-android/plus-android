...
public interface QAClient {
    ...
    Observable<BaseJsonV2<QAListInfoBean>> publishQuestion(@Body RequestBody body);
    ...
    Observable<BaseJsonV2<QAListInfoBean>> uplaodQuestion(@Path("question") Long question_id, @Body RequestBody body);
     ...
     * @param offset 获取 id 之后的数据，要获取某条话题之后的数据，传递该话题 ID。
    ...
    Observable<List<QATopicBean>> getQATopic(@Query("name") String name, @Query
            ("offset") Long offset, @Query("follow") Long follow, @Query("limit") Integer limit);
     ...
     * @param after   获取 id 之后的数据，要获取某条问题之后的数据，传递该问题 ID。
     * @param type    默认值 new, all - 全部、new - 最新、hot - 热门、reward - 悬赏、excellent - 精选 。
     * @param id      按照问题 ID 返回问题列表，多个采用半角 , 进行分割。
     */
    @GET(ApiConfig.APP_PATH_GET_QUESTIONS_LSIT)
    Observable<List<QAListInfoBean>> getQAQustion(@Query("subject") String subject, @Query
            ("offset") Long after, @Query("type") String type, @Query("limit") Integer limit, @Query("id") String id);
    ...
    Observable<List<SimpleAnswerBean>> getSimpleAnswerList(@Query("id") String id);
    ...
    Observable<BaseJsonV2<Object>> managerDeleteQuestion(@Path("question") String question_id);
    ...
    Observable<BaseJsonV2<Object>> applyForExcellent(@Path("question") String question_id,@Field("password") String psd);
    ...
    Observable<BaseJsonV2<AnswerInfoBean>> payForOnlook(@Path("answer_id") Long answer_id,@Field("password") String psd);
    ...
    Observable<Object> rewardQA(@Path("answer_id") long answer_id, @Field("amount") long amount, @Field("password") String psd);
    ...
    Observable<BaseJsonV2<Object>> updateQuestionReward(@Path("question") String question_id
            , @Query("amount") int amount,@Field("password") String psd);
