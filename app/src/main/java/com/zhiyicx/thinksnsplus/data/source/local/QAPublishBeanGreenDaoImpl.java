...
public class QAPublishBeanGreenDaoImpl extends CommonCacheImpl<QAPublishBean> {
    ...
    public long saveSingleData(QAPublishBean singleData) {
        if (singleData != null) {
            return getWDaoSession().getQAPublishBeanDao().insertOrReplace(singleData);
        }
        return -1L;
    ...
    public void saveMultiData(List<QAPublishBean> multiData) {
        getWDaoSession().getQAPublishBeanDao().loadAll();
    }
    ...
    public void clearTable() {
        getWDaoSession().getQAPublishBeanDao().deleteAll();
    }

    @Override
    public void deleteSingleCache(Long primaryKey) {
        getWDaoSession().getQAPublishBeanDao().deleteByKey(primaryKey);
    }
