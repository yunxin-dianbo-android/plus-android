...
public class SendDynamicDataBeanV2 implements Serializable, Parcelable {
    ...
    private String repostable_type;// 可选，如果 repostable_id 存在则必须，转发资源类型标识。
    private Long repostable_id;// 可选，如果 repostable_type 存在则必须，转发资源 ID。
    private Long amount;
    ...
    @Convert(converter = IntegerListConvert.class, columnType = String.class)
    private List<Integer> topics;// 话题
    @Transient
    private List<TopicListBean> topicListBeans;// 话题

    public List<TopicListBean> getTopicListBeans() {
        return topicListBeans;
    }

    public void setTopicListBeans(List<TopicListBean> topicListBeans) {
    ...
    public static SendDynamicDataBeanV2 DynamicDetailBean2SendDynamicDataBeanV2(DynamicDetailBeanV2 dynamicBean) {
        ...
        if (dynamicBean.getMLetter() != null) {
            sendDynamicDataBeanV2.setRepostable_id(Long.parseLong(dynamicBean.getMLetter().getId()));
            sendDynamicDataBeanV2.setRepostable_type(dynamicBean.getMLetter().getType());
        }
