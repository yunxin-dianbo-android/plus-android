...
//    }
    ...
    public String getFriendlyContent() {
        if (friendlyContent == null && feed_content != null) {
            friendlyContent = feed_content.replaceAll(MarkdownConfig.NETSITE_FORMAT, Link
                    .DEFAULT_NET_SITE);
...
//            friendlyContent = friendlyContent.replaceAll(MarkdownConfig.IMAGE_FORMAT, "图片");
    ...
    private void dealImageBean(ImagesBean imageBean, int i, int imageCount) {
        ...
        proportion = (proportion > 100 || isGif) ? IMAGE_100_ZIP : proportion;
        ...
        if (imageCount == 1) {
            ...
            if (proportion > 100 || isGif) {
                proportion = 100;
            }
            imageBean.setGlideUrl(ImageUtils.imagePathConvertV2(canLook, imageBean.getFile(), canLook ? with : 0, canLook ? height : 0
                    , proportion, AppApplication.getTOKEN()));
    ...
    protected DynamicDetailBeanV2(Parcel in) {
        ...
        this.hot = (Integer) in.readValue(Integer.class.getClassLoader());
        ...
        this.topics = in.createTypedArrayList(TopicListBean.CREATOR);
