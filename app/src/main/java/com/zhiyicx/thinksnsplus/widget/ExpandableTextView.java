...
public class ExpandableTextView extends AppCompatTextView {
    ...
    private void init() {
        ...
        if (TextUtils.isEmpty(mToShrinkHint)) {
            mToShrinkHint = getResources().getString(mIsNeedShrink?R.string.qa_topic_close_all:R.string.empty);
        }
        if (mToggleEnable) {
    ...
    private CharSequence getNewTextByConfig() {
        ...
        switch (mCurrState) {
            case STATE_SHRINK: {
                ...
                String fixText = removeEndLineBreak(mOrigText.subSequence(0, indexEnd));
