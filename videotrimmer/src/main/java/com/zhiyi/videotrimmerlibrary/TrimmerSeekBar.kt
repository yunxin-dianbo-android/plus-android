...
class TrimmerSeekBar : View {
    ...
    private val minTrimmerThumbTime = Constant.DEFAULT_TRIMMER_MIN_TIME
    var perThumbWidth = 0
    private var side = ""
    ...
    private fun initialPaints() {
        ...
        initialTrimmerPaint()
        initialCursorPaint()
        setBitmap(MediaHandleManager.getInstance().getConfigVo().leftCursorBitmap,
                MediaHandleManager.getInstance().getConfigVo().rightCursorBitmap)
    }
    ...
    fun setBitmap(leftRes: Int, rightRes: Int) {
        if (leftRes <= 0) {
            return
        }
    ...
    private fun actionMoveLeft(movingX: Float): Boolean {
        ...
        val canNotMove = RegulatorControl.getInstance().timeEnd - RegulatorControl.getInstance().timeStart <= minTrimmerThumbTime
        if ((!canNotMove || movingX < actionDownPosX) && (movingX in 0f..(rightPosX - perThumbWidth * minTrimmerThumbCount))) {
            // movingX in 0f..(rightPosX - perThumbWidth * minTrimmerThumbCount
            leftPosX = movingX
            postInvalidate()
            return true
        }
    ...
    private fun actionMoveRight(movingX: Float): Boolean {
        ...
        val canNotMove = RegulatorControl.getInstance().timeEnd - RegulatorControl.getInstance().timeStart <= minTrimmerThumbTime
        if ((!canNotMove || movingX > actionDownPosX) && movingX in (leftPosX + perThumbWidth * minTrimmerThumbCount)..width.toFloat()) {
            // movingX in (leftPosX + perThumbWidth * minTrimmerThumbCount)..width.toFloat()
            rightPosX = movingX
            postInvalidate()
            Log.d("actionMoveRight::", rightPosX.toString())
            return true
        }
        return false
    }
    ...
    private fun drawCursor(canvas: Canvas) {
        if (leftCursorBitmap != null && rightCursorBitmap != null) {
            canvas.drawBitmap(leftCursorBitmap, leftPosX, 0f, null)
            canvas.drawBitmap(rightCursorBitmap, rightPosX - cursorWidth, 0f, null)
            return
        }

        if ((leftCursorBitmap == null || rightCursorBitmap == null) && leftCursorRect != null && rightCursorRect != null) {
