...
public class SideBar extends TextView {
     ...
     * 缩放离原始的宽度
     */
    private float scaleWidth = dp(10);

    private float itemPaddingTop = dp(6);
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            scaleTime = typedArray.getFloat(R.styleable.SideBar_scaleTime, 1);
        ...
        bigTextPaint.setTextSize(getTextSize() + getTextSize() * (scaleTime + 1f));
        bigTextPaint.setTextAlign(Paint.Align.CENTER);
        scaleTextPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        scaleTextPaint.setColor(getCurrentTextColor());
        scaleTextPaint.setTextSize(getTextSize() * (scaleTime + 0.5f));
        ...
        if (y != 0) {
            for (int i = 0; i < letters.length; i++) {
                ...
                if (y >= currentItemY && y < nextItemY) {
                    ...
                    canvas.drawText(letters[i], w - getPaddingRight() - scaleWidth - bigTextSize, singleTextH + itemH * i + itemPaddingTop, bigTextPaint);
                }
            }
        }
...
//        }
        if (index == -1) {
            ...
            for (int i = 0; i < letters.length; i++) {
                canvas.drawText(letters[i], w - getPaddingRight(), singleTextH + itemH * i + itemPaddingTop, textPaint);
            }
            //触摸的时候画缩放图
        } else {
            //遍历所有字母
            for (int i = 0; i < letters.length; i++) {
                ...
                //超出边界直接花在边界上
                if (drawX > maxRightX)
                    canvas.drawText(letters[i], maxRightX, singleTextH + itemH * i + itemPaddingTop, textPaint);
                else
                    canvas.drawText(letters[i], drawX, singleTextH + itemH * i + itemPaddingTop, scaleTextPaint);
